...
def init():
    ...
    initShaders3( )
    initShaders4( )
    gtex = textureFold()                                    # groupe de textures
    glActiveTextureARB(GL_TEXTURE0_ARB)                     # texture 3D id 0
    im3d, sz, nbtex3d, filt = texture3D(gtex[scen], 20)     # charge scene par defaut
    ...
    sizedp[0]=4*int(((float(f[0])/2.0)+0.5)*200.0)          # read resized kinect
...
def souris(but, sta, x , y):
    global onsl
    onsl = -1
    gly = -1.0*((3.0*y*ratioT/768.0)-(1.5*ratioT))
    glx = x*(3/1366.0)-1.5
